...
void AES::aesEncrypt(const uint8_t key[16], const uint8_t input[16], uint8_t output[16]) {
	uint32_t roundKey[44];
	for (int i = 0; i < 4; i++) {
		roundKey[i] = (uint32_t)(key[i * 4]) << 24 | (uint32_t)(key[i * 4 + 1]) << 16 | (uint32_t)(key[i * 4 + 2]) << 8 | (uint32_t)(key[i * 4 + 3]);
	}
...
void AES::ctrEncrypt(uint8_t *key, uint8_t *nonce, uint16_t len, uint8_t *cipherText, uint8_t *plainText) {
	uint32_t counter = 0;
	uint8_t counterIV[16];
...
void AES::cbcMAC(uint8_t *key, uint8_t *input, uint16_t len, uint8_t *mac) {
	memset(mac, 0, 16);
	for (int i = 0; i < 32 + len; i += 16) {
		uint8_t processingMAC[16];
