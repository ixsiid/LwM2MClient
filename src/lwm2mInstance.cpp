...
void LwM2MInstance::addResource(int resourceId, Operations operation, ResourceType type) {
	resources->add(resourceId, new Resource(resourceId, operation, type));
}

void LwM2MInstance::registCallback(int resourceId, std::function<void(Operations operation, TLVData* tlv, int dataLength)> callback) {
...
Coap::Code LwM2MInstance::writeAll(const uint8_t* buffer, int length, bool bootstrapping) {
	...
	int index		   = 0;
	while (index < length) {
		int id	  = Resource::getTLVId(&buffer[index]);
		Resource* target = (Resource*)resources->find(id);
		if (target) {
			result = Coap::Code::MethodNotAllowed;

			if (bootstrapping || target->isWrite()) {
				index += target->parse(&buffer[index]);
				target->update();
			} else {
				index += Resource::calculateParseLength(&buffer[index]);
			}
		}
	}
