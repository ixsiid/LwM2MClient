...
const static uint32_t initialValue[8] = {
...
void SHA256::sha256Transform(const uint32_t *data) {
	uint32_t calc[8];
	uint32_t s0, s1;
	uint32_t T1, T2, *W256;

	W256 = (uint32_t *)buffer;
	memcpy(calc, state, 32);

	for (int i = 0; i < 16; i++) {
		W256[i] = getReverseUint32(*data++);
		T1	   = calc[7] + (((calc[4] >> 6) | (calc[4] << (32 - 6))) ^ ((calc[4] >> 11) | (calc[4] << (32 - 11))) ^ ((calc[4] >> 25) | (calc[4] << (32 - 25)))) + ((calc[4] & calc[5]) ^ (~calc[4] & calc[6])) + K256[i] + W256[i];

		T2 = (((calc[0] >> 2) | (calc[0] << (32 - 2))) ^ ((calc[0] >> 13) | (calc[0] << (32 - 13))) ^ ((calc[0] >> 22) | (calc[0] << (32 - 22)))) + ((calc[0] & calc[1]) ^ (calc[0] & calc[2]) ^ (calc[1] & calc[2]));
	...
	for (int i = 16; i < 64; i++) {
		...
		T1 = calc[7] + (((calc[4] >> 6) | (calc[4] << (32 - 6))) ^ ((calc[4] >> 11) | (calc[4] << (32 - 11))) ^ ((calc[4] >> 25) | (calc[4] << (32 - 25)))) + ((calc[4] & calc[5]) ^ (~calc[4] & calc[6])) + K256[i] + W256[i & 0x0f];
...
void SHA256::sha256Update(const uint8_t *data, size_t len) {
	...
	if (usedspace > 0) {
		...
		if (len >= freespace) {
			...
			len -= freespace;
			data += freespace;
			sha256Transform((uint32_t *)buffer);
		} else {
...
void SHA256::sha256End(const uint8_t digest[32]) {
	...
	if (digest != (uint8_t *)0) {
		...
		*(uint64_t *)&buffer[SHORT_BLOCK_LENGTH] = bitcount;
...
void SHA256::sha256(const uint8_t *input, size_t len, const uint8_t *output) {
	sha256Init();
	sha256Update(input, len);
	sha256End(output);
}

void SHA256::hmacSha256(const uint8_t *secret, int secretLen, const uint8_t *message, int messageLen, const uint8_t *output) {
	uint8_t secretBuf[64];
	memset(secretBuf, 0, 64);
	memcpy(secretBuf, secret, secretLen);
	uint8_t secretIPad[64];
	uint8_t secretOPad[64];
	for (int i = 0; i < 64; i++) {
		secretIPad[i] = secretBuf[i] ^ 0x36;
		secretOPad[i] = secretBuf[i] ^ 0x5c;
	}
	uint8_t secretMessage[64 + messageLen];
	memcpy(secretMessage, secretIPad, 64);
	memcpy(&secretMessage[64], message, messageLen);
	uint8_t secretIPadHash[32];
	sha256(secretMessage, 64 + messageLen, secretIPadHash);
	uint8_t secretJoined[96];
